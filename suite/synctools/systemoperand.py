...
for line in lines:
    ...
    if line.strip() == 'constexpr MorelloCSysReg MorelloCSysRegsList[] = {':
        count += 1
        print_line('static const MorelloCSysReg MorelloCSysRegsList[] = {')
        continue
...
# lookupMorelloCSysRegByEncoding
count = 0
for line in lines:
    line = line.rstrip()
    ...
    if 'lookupMorelloCSysRegByEncoding' in line and '{' in line:
        count += 1
        print_line('const MorelloCSysReg *lookupMorelloCSysRegByEncoding(uint16_t Encoding)\n{')
        print_line('  unsigned int i;')
        continue
...
print_line("""
  ...
  if (i == -1)
    ...
    return &MorelloCSysRegsList[Index[i].index];
}
""")
